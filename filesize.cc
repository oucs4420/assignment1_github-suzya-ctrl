...
int main(int argc, char *argv[])
{
    ...
    {                             // increments the amount of arguements to get through all the files if there is more than 1
        int counter = 0;          // represents amount of lines in the file
        filename.open(argv[arg]); // opens the file

        if (filename.fail())
        {                                               // checks if file has failed to open/unreadable
            cout << " " << argv[arg] << ": -1" << endl; // if the file doesnt open a -1 is outputted to the console
        }
        else
        {
            while (getline(filename, line))
            {              // if the file is openable goes through each line
